...
AboutView::AboutView()
{
    ...
    body.setText (
        ...
        "\n"
        "Resample  -  in a pad\'s right-click bubble, bounces the whole pattern -- kit, "
        "rolls, macros, EQ, the lot -- onto that pad as ONE seamless loop. The decay that "
        "runs past the last bar is folded back over the start, where the loop\'s next pass "
        "would have put it, so it repeats without a seam and is exactly one pattern long. "
        "Every take is saved to its own file under Resamples/ and never overwrites the last. "
        "The pad you bounce onto plays its own old sound into the bounce, so pick an unused "
        "one unless that is what you want.\n"
        "\n"
        ...
        "\n"
        "Library  -  point FOLDERS at your samples once. Everything under it is analysed "
        "in the background -- the window stays alive, and a progress line counts them "
        "off -- and anything you drop into that folder later is picked up on its own, "
        "with no rescan to remember. The folders are saved, so it is still watching them "
        "tomorrow. Stop watching one and its samples stay: they are still on your disk. "
        "Click a sample to hear it. Right-click one to send it to a pad, slice it across "
        "the pads, or re-tag its category; the correction sticks and survives a re-scan. "
        "NEW KIT builds a playable kit, and no sample is dealt to two pads unless its "
        "category has fewer sounds than pads asking for one. Swapping a pad's sample "
        "works while the beat is playing.\n"
        "\n"
