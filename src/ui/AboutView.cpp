...
AboutView::AboutView()
{
    ...
    body.setText (
        ...
        "\n"
        "Capture (REC)  -  record what you play straight onto the grid, quantised. Press Play, "
        "then REC; capture needs the transport running, because it times every hit against the "
        "loop. Press REC again to finish, and stopping the transport finishes it too.\n"
        "\n"
        "    PADS  -  every pad you hit lands on the nearest step, and you hear it on the next "
        "pass of the loop. Mouse, the 1234 / QWER / ASDF / ZXCV keys, or a MIDI controller "
        "(note 36 is the first pad, and the next fifteen notes are the rest of the grid).\n"
        "\n"
        "    MIC  -  beatbox into your microphone. The take is chopped at its onsets and each hit "
        "is judged a kick, a snare or a hat, then written to the first three pads. A mouth makes "
        "three drum sounds, so it hears three: it is a sketchpad, not a transcription, and a bad "
        "take is best recorded again rather than argued with. One take is up to 30 seconds. If "
        "MIC is greyed out, the audio device you chose has no input -- pick one in Settings.\n"
        "\n"
        "The whole take is ONE Ctrl/Cmd+Z, however many hits it holds. Hits overdub onto what is "
        "already there, two taps that round onto one step become one note at the louder velocity, "
        "and a hit that lands on a step you had programmed replaces it with a clean tap -- you "
        "played a note, not a roll.\n"        "\n"
        ...
        "the kick.\n"
        "\n"
        "Hats choke each other  -  a closed hat cuts an open one, from the first launch and on "
        "every kit you build. There is no choke group to set up and nothing to route: the hat "
        "pads share one, and any sample you drop on a hat pad joins it.\n"
        "\n"
        ...
        "up = denser). The preset box shapes the roll; Clear Rolls removes every roll in the "
        "pattern, including the one Make a Beat puts at the end of a bar. A roll is drawn as a "
        "solid block across the steps it covers, so you can always see what Clear Rolls will "
        "take away -- and the button greys out when there is nothing to take.\n"
        "\n"
        ...
        "into it, or to clear it. Clear Pattern empties the one you are standing in -- every step "
        "and every roll -- and Ctrl+Z brings it back. Tempo and swing belong to the transport, "
        "not to a slot, so switching never changes them. All eight are saved in a .rollforge "
        "file.\n"
        "\n"
        ...
        "right-click one to remove it; Clear Song empties the whole chain at once (it never "
        "touches the beat). Turn SONG on and "
        ...
        "your DAW.\n"
        "\n"
        "Settings  -  choose the audio device and buffer size, and the UI scale. The INPUT device "
        "chosen here is the one MIC capture records from. Sample folders live in the Library, "
        "under FOLDERS.\n"
        "\n"
