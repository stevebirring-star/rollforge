...
AboutView::AboutView()
{
    ...
    body.setText (
        ...
        "then REC. With the box set to PADS, every pad you hit -- mouse, keys or MIDI -- lands on "
        "the nearest step and you hear it on the next pass of the loop. Set it to MIC and beatbox "
        "into your microphone instead: the take is chopped at its onsets and each hit is judged a "
        "kick, a snare or a hat, then written to pads 1, 2 and 3. A mouth makes three drum sounds, "
        "so it hears three; it is a sketchpad, not a transcription. Capture needs the transport "
        "running, because it times every hit against the loop. The whole take is ONE Ctrl/Cmd+Z. "
        "Hits are overdubbed onto what is already there, and a hit that lands on a step you had "
        "programmed replaces it with a clean tap -- you played a note, not a roll.\n"        "\n"
